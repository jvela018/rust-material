...
echo "-------------------------------------------------------"
echo "Downloading Posgres GIS image!!"
echo "-------------------------------------------------------"
echo "command: ./download-docker-postgres-gis.sh"
./download-docker-postgres-gis.sh

echo "-------------------------------------------------------"
echo "Running Postgres/GIS container"
echo "-------------------------------------------------------"
echo "command: ./run-docker-postgres-gis.sh"
./run-docker-postgres-gis.sh
...
echo "-------------------------------------------------------"
echo " Setup DATABASE_URL, based on the POSTGRES docker image"
echo "-------------------------------------------------------"
echo "command: export DATABASE_URL=postgres://user:password@localhost:5433/rust-iot-db"
...
echo ""
echo "If DATABASE_URL was set, a migrations directory will be created, and under it 00000000000000_diesel_initial_setup
where up.sql and down.sql live. Furthermore a diesel.toml file will be created in the root directory of the project"

echo "----------------------------------------------------------------"
echo "Run migration and generate src/schema.rs defined in diesel.toml"
echo "----------------------------------------------------------------"
echo "command: diesel migration run"
